...
public class OverritePackageAuthInSettopOnDNCSBlock extends SettopBlock
{
    ...
    public OverritePackageAuthInSettopOnDNCSBlock( Settop settop )
    {
        super( settop );
    ...
    public void execute()
    {
        ...
        try
        {
            ...
            boolean result = settop.getDNCSProvider().overwritePackageAuthorizations( channelHandle, true );
            Assert.assertTrue( result );
            /*boolean icResult = getIC().waitForRegion( icRegion_blank, 60000 );
            Assert.assertTrue( icResult );*/
        }
        catch ( DigitalControllerException de )
        {
            log( " Exception while doing OVERWRITE PACKAGE AUTHORISATION on settop {}; Exception is {}", settop.getHostMacAddress(),
                    de.getMessage() );
        }
    }
