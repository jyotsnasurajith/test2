...
public class SettopOnDNCSOverritePackageAuthTest extends SettopBlock
{
    ...
    public SettopOnDNCSOverritePackageAuthTest( Settop settop )
    {
        super( settop );
    ...
    public void execute()
    {
        ...
        // call(QueryDHCTInSettopOnDNCSBlock.class);

        try
        {
            /*
             * I am taking all the packages list here, so that I can revert back
             * to the initial state after performing overwrite package
             * authorisation.
             */
            ...
            int initialCount = queryResponse.getPackageAuthorization().getPackageCount();

            List< RemoteCommand > commands = Arrays.asList( RemoteCommand.THREE, RemoteCommand.SIX );

            /* Tune to channel 36 */
            settop.pressKeys( commands );

            /* Do an overwrite package authorization for package '101' */
            call( OverwritePackageAuthInSettopOnDNCSBlock.class );
            sleep( 40000 );
             ...
             */
            dncsProvider.overwritePackageAuthorizations( packages, true );
            QueryDHCTResponse response = dncsProvider.invokeQueryDHCT();
            int finalCount = response.getPackageAuthorization().getPackageCount();
            log("InitialCount of packages (Before OverwriteOperation) " + initialCount);
            log("finalCount of packages (After OverwriteOperation) " + finalCount);
